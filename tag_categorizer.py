...
class TagCategorizer:
    ...
    def _fetch_from_danbooru(self, tags: list):
        ...
        for i in range(0, len(tags), chunk_size):
            ...
            try:
                if session:
                    r = session.get_sync(url, headers={"User-Agent": settings.manager.get_user_agent()})
                else:
                    import requests
                    r = requests.get(url, headers={"User-Agent": settings.manager.get_user_agent()}, timeout=5.0)
