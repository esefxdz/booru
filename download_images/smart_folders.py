...
def _categorize_post_tags(adapter, post: dict, tags: list) -> dict:
    ...
    try:
        ...
        # categorize_tags is async — run it safely regardless of
        # whether we're already inside an event loop or not
        try:
        ...
        if loop and loop.is_running():
            # Inside an event loop (Qt thread) — offload to a pool
            import concurrent.futures
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
                future = pool.submit(asyncio.run, categorizer.categorize_tags(tags))
