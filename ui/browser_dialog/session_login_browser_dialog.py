###########################################################################################
# ui/browser_dialog/session_login_browser_dialog.py
#
# Used when auto-login fails. Shows the site's real login page and adds a
# "Save & Close" button that the user clicks once they have successfully
# logged in.  The dialog captures the session cookie automatically.
###########################################################################################
...
class SessionLoginBrowserDialog(InAppBrowser):
    ...
    def __init__(self, url, booru_name, target_cookies, parent=None):
        ...
        # Find the toolbar layout (first child widget's layout) and insert the button
        toolbar_layout = self.layout().itemAt(0).widget().layout()
        toolbar_layout.insertWidget(1, self.save_btn)
