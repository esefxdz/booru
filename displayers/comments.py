...
class _FetchThread(QThread):
    ...
    async def _fetch(self) -> list[dict]:
        ...
        if handler is None:
            print(f"[comments] No comment handler for api_type={api_type}")
            return []
    ...
    async def _danbooru(self, session, url, post_id) -> list[dict]:
        ...
        if r.status_code != 200:
            print(f"[comments] Danbooru returned {r.status_code}")
            return []
        data = r.json()
    ...
    async def _gelbooru(self, session, url, post_id) -> list[dict]:
        ...
        if r.status_code != 200:
            print(f"[comments] Gelbooru returned {r.status_code}")
            return []
    ...
    async def _moebooru(self, session, url, post_id) -> list[dict]:
        ...
        if r.status_code != 200:
            print(f"[comments] Moebooru returned {r.status_code}")
            return []
        data = r.json()
        if not isinstance(data, list):
            return []
    ...
    async def _e621(self, session, url, post_id) -> list[dict]:
        ...
        if r.status_code != 200:
            print(f"[comments] e621 returned {r.status_code}")
            return []
        data = r.json()
        if not isinstance(data, list):
            return []
    ...
    async def _philomena(self, session, url, post_id) -> list[dict]:
        ...
        if r.status_code != 200:
            print(f"[comments] Philomena returned {r.status_code}")
            return []
        data = r.json()
