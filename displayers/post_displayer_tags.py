...
class _CategorizerThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            cats = asyncio.run(categorizer.categorize_tags(self._tags))
            self.done.emit(cats)
        except Exception as e:
            print(f"[post_displayer_tags] Categorizer thread failed: {e}")
            self.failed.emit()
...
class ClickableTagsDropdown(CollapsibleWidget):
    ...
    def _tag_clicked(self, tag: str):
        try:
            ...
            print(f"[post_displayer_tags] add_tag failed: {e}")
