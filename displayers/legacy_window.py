...
class DisplayerWindow(QMainWindow):
    ...
    def _check_bookmarked(self):
        ...
        return db.is_post_bookmarked(self.post_id)
    ...
    def _download(self):
        ...
        def work():
            try:
                # Use the global settings manager instead of a hardcoded "files" folder
                dl_dir = settings.manager.get_download_dir()
                ...
                logging.error(f"[legacy_window] Download error: {e}")
    ...
    def _load_image(self, optimized):
        ...
        def work():
            try:
                ...
                logging.error(f"[legacy_window] Image load error: {e}")
        threading.Thread(target=work, daemon=True).start()

    def _show_image(self, data):
        pix = QPixmap()
        if not pix.loadFromData(data):
            logging.error("[legacy_window] Failed to decode image bytes")
    ...
    def _load_gif(self, optimized):
        ...
        def work():
            try:
                ...
                logging.error(f"[legacy_window] GIF load error: {e}")
        threading.Thread(target=work, daemon=True).start()
...
class UniversalViewer:
    ...
    def __init__(self, parent_gui, post):
        ...
        cmd = [sys.executable, "-m", "displayers.legacy_window", file_url, json.dumps(post)]
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        logging.error("Usage: python -m displayers.legacy_window <url> <post_json>")
        sys.exit(1)
